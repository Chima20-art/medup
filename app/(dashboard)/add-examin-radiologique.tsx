...
export default function MergedExamensRadiologiques() {
  ...
  console.log("AudioNote", audioNotes);
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      ...
      Alert.alert("Erreur", "échec de l’enregistrement des données du formulaire , veuillez réessayer n.");
      setIsLoading(false);
      return;
    }
    ...
    for (let audio of audioNotes) {
      try {
        //TODO: upload audio to supabase storage for radiologie
        ...
        console.error("Error processing file:", error, "for file:", audio.id);
      }
    }
    ...
    if (audioFilePaths.length > 0) {
      const { error: updateError } = await supabase
        .from("radiologie")
        .update({ audioNotes: audioFilePaths })
        .eq("id", data[0].id);
