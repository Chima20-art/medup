...
import { ChevronLeft, Ruler, Plus, Trash2 } from "lucide-react-native";
...
export default function Height() {
    ...
    const rulerAnimation = useRef(new Animated.Value(1)).current;

    // Animate ruler
    useEffect(() => {
        const animate = () => {
            Animated.sequence([
                Animated.timing(rulerAnimation, {
                    toValue: 1.2,
                    duration: 500,
    ...
    const fetchValues = async () => {
        ...
        try {
            ...
            Alert.alert("Error", "Failed to fetch height values. Please try again.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDone = async () => {
        if (!value) {
            Alert.alert("Error", "Please enter a valid height value.");
            return;
        }
        try {
            ...
            Alert.alert("Error", "Failed to save height value. Please try again.");
        }
    };
                ...
                { text: "Annuler", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            console.error("Error deleting height value:", error);
                            Alert.alert("Error", "Failed to delete height value. Please try again.");
                        }
                    },
                },
            ]
    ...
    const EmptyState = () => (
        <View className="flex-1 items-center justify-center p-6">
            <Animated.View style={{ transform: [{ scale: rulerAnimation }] }}>
                <Ruler size={64} color="#6366f1" />
            </Animated.View>
            <Text className="text-xl font-semibold text-gray-800 mt-6 text-center">
                Aucune mesure de taille
            </Text>
            <Text className="text-gray-500 text-center mt-2 mb-6">
                Commencez à suivre votre taille en ajoutant votre première mesure
            </Text>
            <TouchableOpacity
            ...
            <View className="px-6 pt-4 pb-4 bg-white shadow-sm">
                <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center">
                        ...
                        <Text className="text-2xl font-bold text-primary-500 ml-2">Taille</Text>
                    </View>
                    <TouchableOpacity
                        onPress={() => setShowAddValue(true)}
                    ...
                    ListHeaderComponent={() => (
                        <>
                            ...
                            <View className="m-4 p-6 bg-white rounded-2xl shadow-md">
                                <Animated.View
                                    style={{
                                        transform: [{ scale: rulerAnimation }],
                                        alignItems: 'center'
                                    }}
                                >
                                    <Ruler size={36} color="#ef4444" />
                                </Animated.View>
                                <View className="items-center mt-6 py-6">
                                    <View className="flex-row items-end">
                                        ...
                                        <Text className="text-xl text-gray-500"> cm</Text>
                                    </View>
