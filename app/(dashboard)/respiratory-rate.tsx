...
import { ChevronLeft,Stethoscope, Plus, Trash2 } from "lucide-react-native";
...
export default function RespiratoryRate() {
  ...
  const lungsAnimation = useRef(new Animated.Value(1)).current;

  // Animate lungs
  useEffect(() => {
    const animate = () => {
      Animated.sequence([
        Animated.timing(lungsAnimation, {
          toValue: 1.2,
          duration: 1000,
  ...
  const fetchValues = async () => {
    ...
    try {
      ...
      console.error("Error fetching respiratory rate values:", error);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          { text: "Annuler", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                ...
                console.error("Error deleting respiratory rate value:", error);
              }
            },
          },
        ]
  ...
  const EmptyState = () => (
      <View className="flex-1 items-center justify-center p-6">
        <Animated.View style={{ transform: [{ scale: lungsAnimation }] }}>
          <Stethoscope size={64} color="#6366f1" />
        </Animated.View>
        <Text className="text-xl font-semibold text-gray-800 mt-6 text-center">
          Aucune mesure de fréquence respiratoire
        </Text>
        <Text className="text-gray-500 text-center mt-2 mb-6">
          Commencez à suivre votre fréquence respiratoire en ajoutant votre première mesure
        </Text>
        <TouchableOpacity
                ...
                ListHeaderComponent={() => (
                    <>
                      ...
                      <View className="m-4 p-6 bg-white rounded-2xl shadow-md">
                        <Animated.View
                            style={{
                              transform: [{ scale: lungsAnimation }],
                              alignItems: 'center'
                            }}
                        >
                          <Stethoscope size={36} color="#ef4444" />
                        </Animated.View>
