...
export default function AddConsultation() {
  ...
  const fetchSpecialties = async () => {
    try {
      ...
      if (data) {
        const trimmedData = data.map((specialty) => ({
          ...specialty,
          hexColor: specialty.hexColor.trim(),
        }))
        setSpecialties(trimmedData)
      }
    } catch (error) {
  ...
  const playPauseAudio = async (audioId: string) => {
    ...
    if (selectedAudioId === audioId) {
      ...
      audioNote.sound.setOnPlaybackStatusUpdate((status) => {
        if (status.isLoaded) {
          setAudioNotes((prev) =>
              prev.map((note) => (note.id === audioId ? { ...note, currentTime: status.positionMillis ?? 0 } : note)),
          )
    ...
    try {
      ...
      const { data, error } = await supabase
          ...
          .insert({
            ...
            nextConsultationDateReminder: formData.nextConsultationDateReminder
                ? new Date(formData.nextConsultationDateReminder).toISOString()
                : null,
            nextConsultationDate: formData.nextConsultationDate
                ? new Date(formData.nextConsultationDate).toISOString()
                : null,
      ...
      for (const audio of audioNotes) {
        try {
          const fileExtension = audio.uri.split(".").pop()
          const filePath = `${userId}/${Date.now()}_${fileExtension}`
      ...
      if (formData.reminder && formData.nextConsultationDateReminder) {
        const generatedRandomId: string = Array.from(Array.from({ length: 16 }, () => Math.floor(Math.random() * 256)))
            .map((b) => b.toString(16).padStart(2, "0"))
            .join("")

        const notifaction = scheduleNotification(
            generatedRandomId,
        ...
        const { error: updateError } = await supabase
            .from("consultations")
            .update({
              notificationId: [generatedRandomId],
            })
            .eq("id", data[0].id)
  ...
  const renderSpecialtyPicker = () => (
      <View className="absolute z-10 top-full left-0 right-0 bg-white rounded-b-xl shadow-lg max-h-48">
        {/* Use a regular map instead of FlatList to avoid nesting VirtualizedLists */}
  ...
  return (
      <SafeAreaView className="flex-1 bg-gray-50  pb-0">
        <View className="px-6 pt-2 pb-2 bg-white">
          <View className="flex flex-row items-center">
            ...
            <Text className="text-primary-500 text-[20px] font-extrabold ml-4">Ajouter une consultation</Text>
          </View>
        </View>
        ...
        >
          <ScrollView className="flex-1 p-6 pb-16 ">
            <View className="flex flex-col gap-y-4 pb-10">
              {/* Doctor Name Input */}
              <View>
                <Text className="text-md font-medium text-gray-700 mb-1">Nom du médecin</Text>
              ...
              {/* Specialty Selector */}
              <View className="z-50">
                <Text className="text-md font-medium text-gray-700 mb-1">Spécialité</Text>
                <View className="relative">
                  ...
                  >
                    <View className="flex-row items-center flex-1">
                      ...
                      {selectedSpecialty ? (
                          <View
                              style={{ backgroundColor: selectedSpecialty.hexColor }}
                              className="px-3 py-1 rounded-full ml-3"f
                          >
              ...
              {/* Date Input */}
              <View>
                <Text className="text-md font-medium text-gray-700 mb-1">Date de réalisation</Text>
                <TouchableOpacity
                ...
                >
                  ...
                  <Text className="flex-1 ml-3 text-gray-700">{formatDate(formData.date)}</Text>
                </TouchableOpacity>
                {showDatePicker && (
              ...
              {/* Address Input */}
              <View>
                <Text className="text-md font-medium text-gray-700 mb-1">Lieu de réalisation</Text>
                <View className="flex-row items-center bg-white rounded-xl border border-gray-200 px-4 h-14">
                  <MapPin size={20} color={colors.text} className="opacity-50" />
                  <TextInput
                      value={formData.adress}
                      onChangeText={(text) => setFormData((prev) => ({ ...prev, adress: text }))}
                      placeholder="Adresse"
                      placeholderTextColor="#9CA3AF"
                      className="flex-1 ml-3 text-md font-semibold"
                  />
              ...
              {/* City Input */}
              <View>
                <Text className="text-md font-medium text-gray-700 mb-1">Ville</Text>
                <View className="flex-row items-center bg-white rounded-xl border border-gray-200 px-4 h-14">
              ...
              <View className="mt-4 bg-indigo-100 rounded-2xl p-2 flex-xol gap-y-2">
                ...
                <View>
                  <Text className="text-md font-medium text-gray-700 mb-1">Date de la prochaine consultation</Text>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text className="flex-1 ml-3 text-gray-700">
                      {formData.nextConsultationDate
                          ? new Date(formData.nextConsultationDate).toLocaleString("fr-FR", {
                            year: "numeric",
                            month: "long",
                            day: "numeric",
                            hour: "2-digit",
                  ...
                  {showNextConsultationDatePicker && (
                      <View className="z-10 mt-1 w-full bg-white rounded-xl shadow-lg">
                          ...
                          <View className="bg-white rounded-xl border border-gray-200 p-4">
                            ...
                            {formData.nextConsultationDateReminder && (
                                <Text className="pl-2 pt-2">
                                  Rappel: le{" "}
                                  {new Date(formData.nextConsultationDateReminder).toLocaleString("fr-FR", {
                                    year: "numeric",
                                    month: "long",
                                    day: "numeric",
                                    hour: "2-digit",
                        ...
                        {/* File Upload Section */}
                        <View>
                          <Text className="text-md font-medium text-gray-700 mb-1">Documents</Text>
                          <View className="bg-white rounded-xl border border-gray-200 p-4">
                ...
                {/* Reminder Section */}
                <View>
                  <Text className="text-md font-medium text-gray-700 mb-1">Rappel pour la prochaine consultation</Text>
                  <View className="bg-white rounded-xl border border-gray-200 p-4">
                    <View className="flex-row items-center justify-between mb-4">
                      <Text className="text-gray-700">Activer le rappel</Text>
                      <Switch
                        ...
                        {audioNotes.map((audio) => (
                            ...
                            >
                              ...
                              >
                                ...
                                <View className="flex-1">
                                  <Text className="text-md text-gray-600">Note audio {audioNotes.indexOf(audio) + 1}</Text>
                                  <Text className="text-xs text-gray-400">
                                    {selectedAudioId === audio.id
                                        ? `${formatTime(audio.currentTime)} / ${formatTime(audio.duration)}`
                                        : formatTime(audio.duration)}
        ...
        <View className="p-6 pb-2 bg-white border-t border-gray-200">
          <TouchableOpacity
              onPress={handleSubmit}
              disabled={isLoading}
              className={`w-full rounded-xl py-3 items-center ${isLoading ? "bg-primary-400" : "bg-primary-500"}`}
          >
            <Text className="text-white font-semibold text-xl">{isLoading ? "Chargement..." : "Enregistrer"}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
  )
