...
export default function BloodGlucose() {
    ...
    useEffect(() => {
        ...
        animate()
    }, [dropletAnimation]) // Added dropletAnimation to dependencies
    ...
    const handleDelete = async (id: number) => {
        Alert.alert("Supprimer la mesure", "Êtes-vous sûr de vouloir supprimer cette mesure ?", [
            { text: "Annuler", style: "cancel" },
            {
                ...
                onPress: async () => {
                    try {
                        const { error } = await supabase.from("blood glucose").delete().eq("id", id)
                        if (error) throw error
                        await fetchValues()
                    } catch (error) {
                        console.error("Error deleting blood glucose value:", error)
                        Alert.alert("Error", "Failed to delete blood glucose value. Please try again.")
                    }
                },
            },
        ])
    ...
    const EmptyState = () => (
        <View className="flex-1 items-center justify-center p-6">
            ...
            <Text className="text-xl font-semibold text-gray-800 mt-6 text-center">Aucune mesure de glycémie</Text>
            ...
            <View className="px-6 pt-4 pb-4 bg-white shadow-sm">
                <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center">
                        ...
                        <Text className="text-2xl font-bold text-primary-500 ml-2">Glucose sanguin</Text>
                    </View>
                    <TouchableOpacity
                        onPress={() => setShowAddValue(true)}
