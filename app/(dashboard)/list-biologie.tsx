...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Animated, SafeAreaView } from "react-native"
import { useRouter } from "expo-router"
import { Calendar, Search, Building2, User2, Plus, ChevronLeft } from "lucide-react-native"
...
export default function ListExaminsBiologiques() {
  ...
  const fetchRadiologies = async () => {
    const { data, error } = await supabase.from("bilogie").select("*").order("created_at", { ascending: false })
  ...
  const handleDeleteExamen = (id: number) => {
    setRadiologies((prevRadiologies) => prevRadiologies.filter((radio) => radio.id !== id))
  }
            ...
            <View className="flex-col flex-1 mr-2">
              <View className="flex-row items-start gap-x-2">
                ...
                <View className="flex flex-col items-start h-full  mr-2 flex-1">
                  <Text className="text-primary-500 text-[24px] font-extrabold">Mes examens {"\n"} biologiques</Text>
                </View>
              </View>
              {/* Search Bar */}
        ...
        <ScrollView className="flex-1 px-4 pt-4">
          {radiologies.length === 0 ? (
              <View className="flex-1 items-center justify-center py-8">
                <Text className="text-gray-500 text-lg font-medium">Aucun examen biologique trouvé</Text>
              </View>
          ) : (
                ...
                return (
                    ...
                    >
                      ...
                      <View className="flex-1 bg-white rounded-3xl p-4 shadow-sm">
                        <Text className="text-gray-900 text-lg font-bold mb-2">{bio.name}</Text>

                        {/* Laboratory Info */}
                        <View className="flex-row items-center mb-2 w-fit ">
                          <Hospital />
                          <Text className="text-base font-semibold text-gray-600 ml-2">{bio.labName}</Text>
                        </View>

                        {/* Doctor Info */}
                        <View className="flex-row items-center mb-2 w-fit">
                          <Doctor />
                          <Text className="text-base font-semibold text-gray-600 ml-2">{bio.phone}</Text>
                        </View>
                        ...
                        <View className="flex-row gap-x-2 justify-end">
                          ...
                          >
                            <Text className="text-xs font-medium text-secondary">
                              {bio?.uploads?.length ? `${bio.uploads?.length} fichier(s)` : "Aucun fichier"}
                            </Text>
                          </TouchableOpacity>
                        </View>
                      </View>
